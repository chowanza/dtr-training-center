...
function RoleNode({ role, selectedId }: { role: Role; selectedId?: string }) {
  const children = roleChildren(role.id);
  const people = usersInRole(role.id);
        ...
        <div className="flex gap-8 pt-6">
          {children.map((c) => (
            <div key={c.id} className="flex flex-col items-center">
              <RoleNode role={c} selectedId={selectedId} />
            </div>
          ))}
        </div>
      )}
...
export default async function RoleChartPage({ searchParams }: { searchParams: Promise<{ role?: string }> }) {
  ...
  const roots = roleChildren(null);
  const selected = selectedId ? db.roles.find((r) => r.id === selectedId) : undefined;
  const responsibilities = selected ? db.responsibilities.filter((r) => r.roleId === selected.id).sort((a, b) => a.sortOrder - b.sortOrder) : [];
  const people = selected ? usersInRole(selected.id) : [];
      ...
      <div className="grid lg:grid-cols-[1fr_320px] gap-6">
        <div className="border border-rule rounded-xl bg-surface p-8 overflow-x-auto">
          <div className="flex gap-16 justify-center min-w-max">
            {roots.map((r) => (
              <RoleNode key={r.id} role={r} selectedId={selectedId} />
            ))}
          </div>
        </div>
