...
export default async function GroupsPage() {
  ...
  const { groups, groupMembers } = await withTenantContext(orgId, async (tx) => {
    const groups = await tx.select().from(schema.groups).where(eq(schema.groups.organizationId, orgId));
    const groupMembers = await tx.select().from(schema.groupMembers).where(eq(schema.groupMembers.organizationId, orgId));
    ...
    <div>
      ...
      <div className="grid md:grid-cols-2 gap-6">
        <div className="border border-rule rounded-xl bg-surface divide-y divide-rule">
          ...
          {groups.length === 0 && <p className="px-5 py-6 text-sm text-ink-2">No groups yet.</p>}
        </div>
