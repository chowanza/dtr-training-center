...
export default async function BuilderIndexPage({ searchParams }: { searchParams: Promise<{ tab?: string; q?: string }> }) {
  ...
  const canAuthor = viewer.accessRole === "admin" || viewer.accessRole === "editor";

  const { modules, moduleVersions } = await withTenantContext(viewer.organizationId, async (tx) => {
    const modules = await tx.select().from(schema.modules).where(eq(schema.modules.organizationId, viewer.organizationId));
    const moduleVersions = await tx.select().from(schema.moduleVersions).where(eq(schema.moduleVersions.organizationId, viewer.organizationId));
    return { modules, moduleVersions };
  });

  let filteredModules = modules;
  if (tab === "published") filteredModules = filteredModules.filter((m) => m.status === "published");
  if (tab === "draft") filteredModules = filteredModules.filter((m) => m.status !== "published");
  if (q) filteredModules = filteredModules.filter((m) => m.title.toLowerCase().includes(q.toLowerCase()));

  const completenessByModuleVersionId = new Map<string, Awaited<ReturnType<typeof moduleCompleteness>>>();
  for (const m of filteredModules) {
    const mv = moduleVersions.find((v) => v.moduleId === m.id)!;
    completenessByModuleVersionId.set(mv.id, await moduleCompleteness(viewer.organizationId, mv.id));
  }
  ...
  return (
    <div>
      ...
      <div className="border border-rule rounded-xl bg-surface divide-y divide-rule overflow-hidden">
        {filteredModules.length === 0 && <p className="px-5 py-6 text-sm text-ink-2">No modules match.</p>}
        {filteredModules.map((m) => {
          const mv = moduleVersions.find((v) => v.moduleId === m.id)!;
          const c = completenessByModuleVersionId.get(mv.id)!;
          return (
